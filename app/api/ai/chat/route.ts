...
export async function POST(request: NextRequest) {
  try {
    ...
    // High-priority Q&A overrides to ensure consistency with system context/easter egg
    if (mode === 'public' && typeof userQuery === 'string') {
      const q = userQuery.toLowerCase();

      // 1) Deterministic answer for web developer question (cover variants)
      if (/(siapa|siapakah).*(web\s*developer|developer\s*web|pengembang\s*web|dev\s*web)/i.test(userQuery)) {
        const reply = formatStructuredResponse({
          greeting: '✨ Info Developer',
          mainContent: 'Web developer situs ini adalah Irga Andreansyah Setiawan (Sekbid 6 - Kominfo).',
          followUp: 'Mau lihat info pengurus lain atau sekbid terkait?',
        });
        return NextResponse.json({ reply });
      }

      // 2) Deterministic handling for "Astati an tasbir" meaning and origin (+ variants)
      if (/astati\s*an\s*tasbir/i.test(q) || /kama\s*qoolat/i.test(q) || /dear\s*web\s*developer/i.test(q)) {
        const asksMeaning = /(arti|makna|apa\s+arti)/i.test(userQuery);
        const asksOrigin = /(dari\s+siapa|asal|origin|siapa\s+yang\s+mengatakan|siapa\s+yang\s+beri|siapa\s+yang\s+memberikan)/i.test(userQuery);
        let parts: string[] = [];
        if (asksMeaning) {
          parts.push('Arti frasa tersebut (sesuai konteks personal): "Dia (perempuan) berkata (kepada Irga): Kamu itu bisa sabar."');
        }
        if (asksOrigin || !asksMeaning) {
          parts.push('Asal frasa: dari akun "bilanazhmii", disampaikan untuk Irga Andreansyah Setiawan. ✨');
        }
        const reply = formatStructuredResponse({
          greeting: '📝 Informasi Frasa',
          mainContent: parts.join('\n'),
          followUp: 'Perlu ditampilkan di halaman terkait atau ingin info sekbid Irga?',
        });
        return NextResponse.json({ reply });
      }

      // 3) Salami / sampaikan salam untuk Irga atau bilanazhmii
      if (/(sampaikan|bilang|ucapkan)\s+salam\s+(ke\s+)?(irga|bilanazhmii)/i.test(userQuery)) {
        const toIrga = /(irga)/i.test(userQuery);
      ...
      // 5) Penyebutan alias yang terkait bilanazhmii (The Lady / nanas)
      if (/(the\s+lady|nanas)/i.test(q)) {
        const reply = formatStructuredResponse({
          greeting: '💠 Catatan',
          mainContent: 'Alias yang kamu sebut terkait dengan akun "bilanazhmii". Pesan khususmu akan aku jaga dan sampaikan dengan hangat. 😊',
          followUp: 'Perlu aku rangkumkan pesannya?',
        });
        return NextResponse.json({ reply });
      }
      ...
      if (/(arti|makna)/i.test(q) && /(kata|kata-kata|kutipan)/i.test(q)) {
        const recentText = (baseMessages.slice(-4).map(m => m.content).join(' ') || '').toLowerCase();
        const hasIrgaContext = /(irga|astati|kama\s*qoolat|dear\s*web\s*develover)/i.test(q + ' ' + recentText);
        if (hasIrgaContext) {
          const reply = formatStructuredResponse({
      ...
      // 8) Asal/siapa yang mengatakan kata-kata Irga (umum)
      if (/(dari\s+siapa|asal|origin|siapa\s+yang\s+memberi|siapa\s+yang\s+mengatakan)/i.test(q) && /(kata|kata-kata|kutipan)/i.test(q)) {
        const recentText = (baseMessages.slice(-4).map(m => m.content).join(' ') || '').toLowerCase();
        const hasIrgaContext = /(irga|astati|kama\s*qoolat|dear\s*web\s*develover)/i.test(q + ' ' + recentText);
        if (hasIrgaContext) {
          const reply = formatStructuredResponse({
